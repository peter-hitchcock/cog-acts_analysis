CleanUpPSTStudy1Main <- function(PST_raw, train_df) {
  ...
  #te <- PST_raw %>% filter(key_presses == 999)
  te <- PST_raw
  if (any(unlist(lapply(te$rt, is.null))==TRUE)) {
    te <- te[-which(unlist(lapply(te$rt, is.null))), ] 
  }
  ...
  unique(print(unique(unlist(te$participant_id))))
  test <- data.frame(
  ...
  test[test$test_condition=="cogn_overt", "test_condition"] <- "overt_cogn"
  # Confirmed that this is matched contingencies only — 
  # test[test$test_condition=="overt_cogn", ] 
  ...
  test_df_new$rt <- as.numeric(unlist(te$rt))
