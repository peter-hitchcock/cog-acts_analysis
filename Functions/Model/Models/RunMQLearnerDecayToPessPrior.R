RunMQLearnerDecayToPessPrior <- function(parameters, train_df, helpers, SIT_df=NULL, mle_means=NULL, mle_covar=NULL) { 
  ### Q-learner with decay to a pess prior ###
  ...
  for (tr in 1:length(states)) {
    ...
    # Test LSE approx fx — confirm always within an epsilon of regular softmax (although the regular can over/underflow, which 
    # is need for LSE, so leave commented)
    # cat("\nHere")
    # test_lse_probs <- RunLSESoftmaxWithBiasAndCK(q_vals, beta=beta, bias=0, choice_kernel=c(0, 0)) #+ .01 <-- confirm does break w this
    # test_reg_probs <- unlist(RunSoftMax(q_vals, beta=beta))
    # cat("\nLSE", test_lse_probs)
    # cat("\nReg", test_reg_probs)
    # 
    # if (test_lse_probs[2] < (test_reg_probs[2]-.001)) {
    #   assert(2==1, "\nUnder error")
    # } else if (test_lse_probs[2] > (test_reg_probs[2]+.001)) {
    #   assert(3==1, "\nOver error")
    # } else if (test_lse_probs[1] > (test_reg_probs[1]+.001)) {
    #   assert(3==1, "\nOver error")
    # } else if (test_lse_probs[1] < (test_reg_probs[1]-.001)) {
    #   assert(3==1, "\nUnder error")
    # }
    ...
    # Update the q vals by putting into matrix 
    if (cond == "cog") {
      ...
      # cat("\nStates so far"); print(states[idx:tr])
      # cat("\nCog prior matrix"); print(this_cog_Q_prior)
