PrepProlificDataStudy1Main <- function(js_file, print_length=NULL) {
  ...
  final_out <- lapply(js_file, function(x) ({
    ...
    if (class(x) == "list") {
      ...
      if (!is.na(have_SIT_data)) {
        ...
        # cat("\n SIT"); print(table(unlist(single_item_test_data$valid_response)))
        # cat("\n PST"); print(table(unlist(PST_test_data$validResp)))
        # # #if (table(unlist(PST_test_data < 80)))
        # # #if (table(unlist(PST_test_data$validResp)) < 80) browser()
        # cat("\n Train"); print(table(unlist(train_data$valid_resp)))
      #}
  ...
  if ("train_data" %in% names(all_final_out$results)) {
    train_df_out <- CleanUpTrainStudy1Main(data.frame(all_final_out$results$train_data))  
  ...
  if ("PST_data" %in% names(all_final_out$results)) {
    pst_df_out <- CleanUpPSTStudy1Main(all_final_out$results$PST_data, train_df_out)
  ...
  if ("single_item_test_data" %in% names(all_final_out$results)) {
    single_item_df_out <- CleanUpSingleItemTestStudy1Main(all_final_out$results$single_item_test_data, 
                                                          train_df_out)
