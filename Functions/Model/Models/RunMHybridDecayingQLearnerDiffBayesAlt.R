RunMHybridDecayingQLearnerDiffBayesAlt <- function(parameters, train_df, helpers, SIT_df=NULL, mle_means=NULL, mle_covar=NULL) { 
  ### Hybrid Bayes and Q-learner w Bayes varying by cond — alternate version where uncertainty dictating rho_scaled is at the level of individual 
  # actions instead of averaged over actions ###
  ...
  if (on_what == "train_SIT") {
    ...
    # Calculate the final hybrid values for each state in each cond  
    for (i in 1:4) {
