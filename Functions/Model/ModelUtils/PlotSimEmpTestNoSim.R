PlotSimEmpTestNoSim <- function(emp_df, sim_df, low_range=.1, high_range=.9) {
  ### Returns plots of just the empirical without sims overlaid yet (just for presentation demos) ###
  ...
  x_labs <- unlist(lapply(strsplit(as.character(levels(full_sim_summs$valence_and_probability)), 
    ...
    this_plot <- 
      ...
      xlab("") + ylab("Proportion correct") + tol +
        ...
        theme(axis.text.x = element_text(angle=40, hjust=1, size=25)) +
