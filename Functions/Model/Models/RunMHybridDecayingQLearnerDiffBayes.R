RunMHybridDecayingQLearnerDiffBayes <- function(parameters, train_df, helpers, SIT_df=NULL, mle_means=NULL, mle_covar=NULL) { 
  ### Hybrid Bayes and Q-learner w Bayes varying by cond ###
  ...
  if (on_what == "train_SIT") {
    ...
    for (i in 1:4) {
      ...
      overt_rho_scalar <- NULL
      this_overt_rho <- NULL
      overt_bayes_Q_vals <- NULL
      overt_weighted_uncertainty <- NULL 
      
      cog_a1_row_ideal <- NULL
      cog_a2_row_ideal <- NULL
      cog_arm1_std <- NULL
      cog_arm2_std <- NULL
      cog_uncertainty_mean <- NULL
      cog_rho_scalar <- NULL
      this_cog_rho <- NULL
      ...
      cog_proportion_a1 <- NULL 
      cog_proportion_a2 <- NULL 
      
      cog_proportion_a2 <- 
        sum(cog_a2_row_ideal)/(sum(cog_a1_row_ideal)+sum(cog_a2_row_ideal))
      
      cog_proportion_a1 <- 1-cog_proportion_a2
      
      cog_weighted_uncertainty <- cog_proportion_a2*cog_arm2_std + 
        cog_proportion_a1*cog_arm1_std
      
      cog_rho_scalar <- 1-(cog_weighted_uncertainty/max_uncertainty)
      
      this_cog_rho <- rho*cog_rho_scalar
      
      cog_bayes_Q_vals <- c(
        CalcBayesMean(cog_a1_row_ideal[1], cog_a1_row_ideal[2], si_valence),#, #fill in val),
        CalcBayesMean(cog_a2_row_ideal[1], cog_a2_row_ideal[2], si_valence)
      )
      
      cog_hybrid_Q_vals[i, ] <- (1-this_cog_rho)*cog_Q_mat[i, ] + this_cog_rho*cog_bayes_Q_vals
