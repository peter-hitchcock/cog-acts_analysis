RunMQLearnerDecayTo0Inits <- function(parameters, train_df, helpers, SIT_df=NULL, mle_means=NULL, mle_covar=NULL) { 
  ...
  cog_Q_inits <- cog_Q_mat
  overt_Q_inits <- overt_Q_mat
  ...
  for (tr in 1:length(states)) {
    ...
    if (cond == "cog") {
      ...
      # Decay to initial values of 0 
      cog_Q_mat <- DecayQVals(cog_Q_mat, cog_Q_inits, action, state, phi, debug_info=tr)  
      ...
      overt_Q_mat <- DecayQVals(overt_Q_mat, overt_Q_inits, action, state, phi, debug_info=tr)
