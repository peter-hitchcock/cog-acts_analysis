RunMQLearnerDiffDecayToPessPriorESAndEpsFixed <- function(parameters, train_df, helpers, SIT_df=NULL, mle_means=NULL, mle_covar=NULL) { 
  ### Q-learner with decay — that varies by condition —  to a pess prior. Exploration scalar and epsilon fixed at group estimates ###
  ...
  for (tr in 1:length(states)) {
    ...
    # The above is only true given this project's way of storing the data — so break if that doesn't hold 
    assert(length(unique(conds[1:160])) == 1) 
